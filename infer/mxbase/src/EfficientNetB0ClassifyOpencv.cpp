...
APP_ERROR EfficientNetB0ClassifyOpencv::Init(const InitParam &initParam) {
    ...
    if (ret != APP_ERR_OK) {
        LogError << "EfficientNetB0PostProcess init failed, ret=" << ret << ".";
        return ret;
    }
    return APP_ERR_OK;
}

APP_ERROR EfficientNetB0ClassifyOpencv::DeInit() {
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR EfficientNetB0ClassifyOpencv::ReadImage(const std::string &imgPath, cv::Mat &imageMat) {
    imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
    return APP_ERR_OK;
}

APP_ERROR EfficientNetB0ClassifyOpencv::ResizeImage(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static constexpr uint32_t resizeHeight = 255;
    static constexpr uint32_t resizeWidth = 255;

    cv::resize(srcImageMat, dstImageMat, cv::Size(resizeWidth, resizeHeight));
    return APP_ERR_OK;
}

APP_ERROR EfficientNetB0ClassifyOpencv::CVMatToTensorBase(const cv::Mat &imageMat, MxBase::TensorBase &tensorBase) {
...
APP_ERROR EfficientNetB0ClassifyOpencv::Crop(const cv::Mat &srcImageMat, cv::Mat &dstImageMat) {
    static cv::Rect rectOfImg(15.5, 15.5, 224, 224);
    dstImageMat = srcImageMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR EfficientNetB0ClassifyOpencv::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB0ClassifyOpencv::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB0ClassifyOpencv::SaveResult(const std::string &imgPath,
...
APP_ERROR EfficientNetB0ClassifyOpencv::Process(const std::string &imgPath) {
    cv::Mat imageMat;
